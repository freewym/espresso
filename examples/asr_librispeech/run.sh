...
if [ ${stage} -le 1 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${train_feat_dir}/storage ]; then
    utils/create_split_dir.pl \
      /export/b{10,11,12,13}/${USER}/fairseq-data/egs/asr_librispeech/dump/${train_set}/delta${do_delta}/storage \
      ${train_feat_dir}/storage
  fi
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${valid_feat_dir}/storage ]; then
    utils/create_split_dir.pl \
      /export/b{10,11,12,13}/${USER}/fairseq-data/egs/asr_librispeech/dump/${valid_set}/delta${do_delta}/storage \
...
if [ ${stage} -le 2 ]; then
  ...
  spm_train --bos_id=-1 --pad_id=0 --eos_id=1 --unk_id=2 --input=data/lang/input \
    --vocab_size=$sentencepiece_vocabsize --model_type=$sentencepiece_type \
    --model_prefix=$sentencepiece_model --input_sentence_size=10000000
  ...
  for dataset in $train_set $valid_set $test_set; do
    ...
    if [ "$dataset" == "$train_set" ]; then
      cat $token_text | tr ' ' '\n' | sort | uniq -c | awk '{print $2,$1}' | sort > $dict
      wc -l $dict
    fi
  done
...
if [ ${stage} -le 3 ]; then
  ...
  test_paths=$lmdatadir/$(echo $test_set | sed 's/ /,/g').tokens
...
if [ ${stage} -le 4 ]; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu python3 ../../train.py $lmdatadir --seed 1 \
    ...
    --num-workers 0 --max-tokens 20480 --max-sentences 256 \
...
if [ ${stage} -le 5 ]; then
  ...
  gen_set=test
  for i in $(seq $num); do gen_set="$gen_set test$i"; done
  for gen_subset in $gen_set; do
    ...
    python3 ../../eval_lm.py $lmdatadir --cpu \
      ...
      --max-tokens 40960 --max-sentences 512 --sample-break-mode eos \
...
if [ ${stage} -le 7 ]; then
  ...
  for dataset in $test_set; do
    ...
    CUDA_VISIBLE_DEVICES=$(echo $free_gpu | sed 's/,/ /g' | awk '{print $1}') speech_recognize.py \
      ...
      --path $path --beam 50 --max-len-a 0.1 --max-len-b 0 --lenpen 1.0 \
