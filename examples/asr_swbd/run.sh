...
if [ $stage -le 7 ]; then
  ...
  for dataset in $test_set; do
    ...
    text_opt= && [ "$dataset" == "train_dev" ] && text_opt="--text-files data/$dataset/token_text"
    mkdir -p $decode_dir
    asr_prep_json.py --feat-files $feat $text_opt --utt2num-frames-files $utt2num_frames --output $decode_dir/test.json
    CUDA_VISIBLE_DEVICES=$(echo $free_gpu | sed 's/,/ /g' | awk '{print $1}') speech_recognize.py $decode_dir \
      ...
      --num-shards 1 --shard-id 0 --dict $dict --remove-bpe sentencepiece --non-lang-syms $nlsyms \
