...
class _LookAheadWordLanguageModelDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        left_ranges = [[self.word_pad_idx] * self.max_num_children for _ in range(bsz)]
        right_ranges = [[self.word_pad_idx] * self.max_num_children for _ in range(bsz)]
        for i in range(bsz):
            node = nodes[i]
        ...
        cumsum_probs_children = (
            cumsum_probs.gather(-1, right_ranges) - cumsum_probs.gather(-1, left_ranges)
        ...
        if len(word_idx) > 0:
            ...
            word_probs = torch.where(
                ...
                .squeeze(-1)
                .div_(sum_probs[batch_node_word_end_mask]),
            )  # b x 1
        ...
        out_logprobs[batch_space_mask, :, self.subword_eos_idx] = lm_probs.log_()[
...
class _MultiLevelLanguageModel(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        if wordlm_cached_state is None:  # it is the first time step
            ...
            out_logprobs = (
                self.subwordlm_decoder.get_normalized_probs(
            ...
            wordlm_logprobs[
                batch_space_mask
            ] = self.wordlm_decoder.get_normalized_probs(
                self.wordlm_decoder(w, incremental_state=incremental_state),
                log_probs=True,
                sample=None,
            )[
        ...
        self.set_incremental_state(
            incremental_state, "wordlm_logprobs", wordlm_logprobs
        )
        self.set_incremental_state(
            incremental_state, "subword_cumlogprobs", subword_cumlogprobs
        )
        self.set_incremental_state(incremental_state, "nodes", nodes)
        ...
        word_logprobs += torch.where(
            batch_word_end_mask,
            -subword_cumlogprobs,
            word_logprobs.new([self.log_oov_penalty]),
        )
        ...
        out_logprobs[batch_space_mask, :, self.subword_eos_idx] += wordlm_logprobs[
            batch_space_mask, :, self.word_eos_idx
        ]
