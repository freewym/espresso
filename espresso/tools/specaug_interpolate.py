...
def specaug(
    ...
    freq_masked = freq_mask(
        time_warped, F=F, num_masks=num_freq_masks, pad_value=pad_value
    )
    time_masked = time_mask(
        freq_masked, T=T, num_masks=num_time_masks, p=p, pad_value=pad_value
    )
...
def time_warp(spec, W=5):
    ...
    with torch.no_grad():  # to make the results deterministic
        left = torch.nn.functional.interpolate(
            spec[:, :, :, :center],
            size=(spec.size(2), warped),
            mode="bicubic",
            align_corners=False,
        )
        right = torch.nn.functional.interpolate(
            spec[:, :, :, center:],
            size=(spec.size(2), t - warped),
            mode="bicubic",
            align_corners=False,
        )
...
def time_mask(spec, T=40, num_masks=1, p=0.2, pad_value=0.0):
    """Time masking
